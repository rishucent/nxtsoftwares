...
add_action('wp_enqueue_scripts', 'mynx_scripts');

function mynx_scripts() {
    global $azexo_azh;
    ...
    if (in_array('administrator', (array) $user->roles)) {
        ...
        $frontend_variables['edit_button'] = esc_html__('Edit', 'mynx');
        ...
        if (function_exists('get_redux_instance')) {
            ...
            $frontend_variables['edit_links']['configure_templates'] = array(
                ...
                'text' => esc_html__('Configure template', 'mynx'),
                'target' => '_blank',
            );
        }
    }
    wp_enqueue_script('mynx-frontend', get_template_directory_uri() . '/js/frontend.js', array('jquery', 'imagesloaded'), AZEXO_FRAMEWORK_VERSION, true);
    wp_localize_script('mynx-frontend', 'azexo', $frontend_variables);
    ...
    if (!empty($options['custom-js'])) {
        wp_add_inline_script('mynx-frontend', $options['custom-js']);
    }
...
add_action('wp_enqueue_scripts', 'mynx_styles');

function mynx_styles() {
    global $azexo_azh;
    ...
    if (class_exists('WPLessPlugin')) {
        ...
        wp_enqueue_style('mynx-skin', $skin_style);
    } else {
    ...
    if (isset($options['google_font_families']) && is_array($options['google_font_families'])) {
        ...
        foreach ($options['google_font_families'] as $font_family) {
            if ('off' !== esc_html_x('on', $font_family . ' font: on or off', 'mynx')) {
        ...
        if (!empty($font_families)) {
            ...
            wp_enqueue_style('mynx-fonts', $fonts_url, array(), null);
        }
    }

    wp_enqueue_style('mynx-style', get_stylesheet_uri(), array('mynx-skin'));


    if (!empty($options['custom-css'])) {
        wp_add_inline_style('mynx-style', $options['custom-css']);
    }
    wp_add_inline_style('mynx-style', azexo_dynamic_css());
}

add_action('current_screen', 'mynx_current_screen');

function mynx_current_screen() {
    add_editor_style('editor-styles.css');
}

add_action('init', 'mynx_fix_google_font_families', 13); // after load default skin options

function mynx_fix_google_font_families() {
    $options = get_option(AZEXO_FRAMEWORK);
    if (isset($options['google_font_families']) && is_array($options['google_font_families'])) {
        $font_families = array();
        foreach ($options['google_font_families'] as $font_family) {
